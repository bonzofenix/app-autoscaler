...
DB_EXISTS=$(bosh -d "${BOSH_DEPLOYMENT}" ssh "${POSTGRES_INSTANCE}" \
  -c "sudo su - vcap -c \"PSQL_BIN=\\\$(ls -1d /var/vcap/packages/postgres-*/bin/psql | tail -1) && \\\$PSQL_BIN -h 127.0.0.1 -p 5524 -U ${DB_USER} -d postgres -tAc 'SELECT 1 FROM pg_database WHERE datname='\\'${DEPLOYMENT_NAME}\\'''\"" \
  2>/dev/null | grep -o "1" || echo "")
